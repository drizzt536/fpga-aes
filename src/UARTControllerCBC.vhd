...
architecture UARTControllerCBC_arch of UARTControllerCBC is
	...
	signal IV             : AESBlock  := (others => x"00");
...
begin
	...
	chaining_loop: for i in 0 to 15 generate
		encrypt_iblk(i) <= IV(i) xor iblk(i);
		decrypt_oblk(i) <= IV(i) xor decrypt_output(i);
	end generate;

	oblk <= decrypt_oblk when mode = DECRYPT else encrypt_output;
	TX   <= local_TX;
	...
	debug_LEDs <=
		...
		"10" when others;		-- CTR doesn't have FINALIZING
end architecture;
